  ...
  const renderMessageContent = (content: string) => {
    ...
    return lines.map((line, idx) => {
              ...
              <div key={`table-${idx}`} className="overflow-x-auto my-3 border border-gray-200 dark:border-gray-700 rounded-xl">
                <table className="min-w-full divide-y divide-gray-200 text-xs">
                  <thead className="bg-gray-50 dark:bg-gray-900">
                    <tr>
                      {headers.map((h, i) => (
                        <th key={i} className="px-3 py-2 text-left font-bold text-gray-700 dark:text-gray-300">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700 text-gray-600 dark:text-gray-300">
                    {rows.map((row, rIdx) => (
                      <tr key={rIdx}>
                        {row.map((cell, cIdx) => (
      ...
      const formattedText = parts.map((part, i) => {
        if (i % 2 === 1) {
          return <strong key={i} className="font-extrabold text-gray-900 dark:text-gray-100 bg-emerald-50 dark:bg-emerald-950/40 px-0.5 rounded">{part}</strong>;
        }
        return part;
      });
                ...
                <div key={index} className={`flex ${isAssistant ? 'justify-start' : 'justify-end'}`}>
                  <div className={`max-w-[85%] rounded-2xl p-3 shadow-sm border ${
                    isAssistant 
                      ...
                      : 'bg-emerald-500 text-white border-emerald-400 dark:border-emerald-650 rounded-tr-none'
                  }`}>
                    {isAssistant ? (
