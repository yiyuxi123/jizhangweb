...
export default function AddTransactionModal({ isOpen, onClose, initialTransaction }: { isOpen: boolean, onClose: () => void, initialTransaction?: Transaction, key?: string | number }) {
  ...
  const handleAiImageParse = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        const result = await parseReceiptImage(base64Data, accounts, categories);
        applyAiResult(result);
      } catch (err: any) {
        alert(`图片账单解析失败: ${err.message || err}`);
      } finally {
        setAiLoading(false);
      }
    };
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div id="ai-bookkeeper-section" className="bg-gradient-to-br from-violet-50 to-indigo-50 p-4 rounded-2xl border border-indigo-100 space-y-3">
            ...
            <div className="flex items-center justify-between pt-1">
              <span className="text-[10px] text-gray-400">自动提取交易类型、分类、账户及明细</span>
